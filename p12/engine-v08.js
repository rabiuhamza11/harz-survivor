const CONTRACT = 'walkout-contract v0.8 — state + UI + code + writes + MULTI-MIRROR ANCHOR + IDENTITY walk';

const nacl = (function(){ var module = { exports: {} };
...
// IDENTITY LAYER (P12): Ed25519 authorship — hash seals prove unchanged, signatures prove WHO sealed.
// Private keys NEVER live in sealed bytes: signing keys are passed in by substrate glue; pubkeys are pinned in-chain by key_pin seals.
...
async function canWrite(state) {
  ...
  if (!marker) return { ok: false, reason: 'WRITES SEALED — no active walkout marker (v0.8)' };
